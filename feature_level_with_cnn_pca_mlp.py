"""
feature_level_stacking_complete.py
...
- Three meta-learners: MLP-full, MLP-PCA, PCA-Logistic
...
class MetaLearner:
    ...
    def build_pca_logistic(self, pca_components=0.95):
        """Build PCA + LogisticRegression pipeline - compatible scikit-learn 1.7+"""
        pipeline = Pipeline([
            ...
            ('classifier', LogisticRegressionCV(
                ...
                n_jobs=-1,
                random_state=SEED
                # multi_class is automatically handled in scikit-learn 1.7+
            ))
        ])
        return pipeline
...
class ModelComparator:
    ...
    def train_pca_logistic(self, **kwargs):
        """Train PCA + LogisticRegression"""
        print("\n" + "="*50)
        print("Training PCA + LogisticRegression")
        print("="*50)
        ...
        self.models['PCA-Logistic'] = pca_log
        return pca_log
    ...
    def plot_comparison(self, save_path='diagnostics/model_comparison.png'):
        ...
        axes[0, 0].bar(x - width/2, accuracies, width, label='Accuracy', color='steelblue')
        axes[0, 0].bar(x + width/2, f1_macro, width, label='F1-macro', color='coral')
        ...
        axes[0, 1].bar(models, [4608, 814, 814], color=['steelblue', 'coral', 'forestgreen'])
...
class FeatureStackingPipeline:
    ...
    def _train_models(self):
        ...
        print("\n📌 Training PCA + LogisticRegression...")
    ...
    def _analyze_shap(self):
        """Run SHAP analysis"""
        if not self.args.skip_shap and shap is not None:
            print("\n" + "="*60)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Feature Stacking Pipeline')
